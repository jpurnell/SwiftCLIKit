...
struct MouseEventTests {
    ...
    func parseCtrlModifier() {
        ...
        let event = MouseMode.parse(bytes)
        #expect(event != nil)
        #expect(event?.modifiers.contains(.ctrl) == true)
    }
