...
struct GoSyntaxTokenizer: LanguageTokenizer, Sendable {

    private static let keywords: Set<String> = [
        "func", "var", "const", "type", "struct", "interface", "map", "chan",
        "go", "select", "defer", "range", "package", "import", "return",
        "if", "else", "for", "switch", "case", "fallthrough", "default",
        "break", "continue", "goto", "nil", "true", "false", "iota",
    ]
    ...
    func tokenize(_ line: String, state: inout TokenizerState) -> [StyledSpan] {
        ...
        while !remaining.isEmpty {
            ...
            if remaining.hasPrefix("`") {
                let str = consumeGoRawString(&remaining)
            ...
            // Character literal (single quote in Go is rune literal)
            if remaining.hasPrefix("'") {
                let str = consumeString(&remaining)
                spans.append(StyledSpan(text: str, tokenType: .string))
                continue
...
// MARK: - Go-specific helpers

/// Consumes a raw string literal delimited by backticks (no escape sequences).
private func consumeGoRawString(_ remaining: inout Substring) -> String {
